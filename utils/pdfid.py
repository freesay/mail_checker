...
class cEntropy:
    ...
    def calc(self):
        ...
        if streamCount == 0:
            return (
            allCount, sum(map(lambda x: fEntropy(x, allCount), self.allBucket)), streamCount, None, nonStreamCount,
...
def PDFiD(file, allNames=False, extraData=False, disarm=False, force=False, output=None, disarmed_filebuffers=None,
          filebuffer=None):
        ...
        while byte != None:
            ...
            if charUpper >= 'A' and charUpper <= 'Z' or charUpper >= '0' and charUpper <= '9':
                ...
                (word, wordExact, hexcode, lastName, insideStream) = UpdateWords(word, wordExact, slash, words, hexcode,
                                                                                 ...
                                                                                 oEntropy, fOut, disarmed_pdf_buffer)
                if char == '/':
                    slash = '/'
                else:
                    slash = ''
...
class cPDFiD():
    def __init__(self, xmlDoc, force):
        ...
        self.keywords = {}
        for node in xmlDoc.documentElement.getElementsByTagName('Keywords')[0].childNodes:
            self.keywords[node.getAttribute('Name')] = cCount(int(node.getAttribute('Count')),
...
def ProcessFile(filename, options, plugins, list_of_dict, disarmed_filebuffers=None, filebuffer=None):
    xmlDoc = PDFiD(filename, options.all, options.extra, options.disarm, options.force,
                   disarmed_filebuffers=disarmed_filebuffers, filebuffer=filebuffer)
        ...
        for cPlugin in plugins:
            if not cPlugin.onlyValidPDF or not oPDFiD.errorOccured and oPDFiD.isPDF:
                ...
                if options.csv:
                    ...
                    if not oPDFiD.isPDF:
                        Print(MakeCSVLine((('%s', filename), ('%s', cPlugin.name), ('%s', 'Not a PDF document'))),
                              options)
                else:
                    Print(PDFiD2String(xmlDoc, options.nozero, options.force), options)
...
def PDFiDMain(filenames, options, filebuffers=None):
    ...
    if not filebuffers:
        ...
        for i, filebuffer in enumerate(filebuffers):
            ProcessFile(filenames[i], options, plugins, list_of_dict["reports"], disarmed_buffers["buffers"],
                        filebuffer)
...
def GetOTPParser():
    ...
    oParser.add_option('-b', '--return_disarmed_buffer', action='store_true', default=False,
                       help='Returns the disarmed pdf documents as buffer objects.')
    return oParser
